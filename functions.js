...
const fetchUserInformationFromUsernames = async (usernames) => {
  ...
  /**
 * Updates the quota for a given user in the Google Sheet.
 * 
 * @param {string} name - The name of the user whose quota needs to be updated.
 * @param {number} amountToAdd - The amount of quota to add to the user's current quota.
 * @returns {Promise<{
 *   success: boolean,
  *   message: string
  ...
  */
 async function updateQuota(name, amountToAdd) {
   try {
     const response = await sheets.spreadsheets.values.get({
       spreadsheetId: '1ld74oZ3M1sCIl41111LVzkgf-iu-D9C7KQnj4nMxJTA',  // Replace with your Google Sheet ID
       range: 'Sheet1!A:B',
     });
     ...
     for (let i = 0; i < rows.length; i++) {
       if (rows[i][0] === name) {
         found = true;
         const currentQuota = parseInt(rows[i][1]) || 0;
         const updatedQuota = currentQuota + parseInt(amountToAdd);
         ...
         return { success: true, message: `Updated quota for ${name} to ${updatedQuota}.` };
       }
     }
 
     return { success: false, message: `Name ${name} not found in the sheet.` };
 
   } catch (error) {
     console.error('Error accessing the Google Sheet:', error);
     return { success: false, message: 'Error accessing the Google Sheet.' };
   }
 };
